import { Resolver, Arg, Mutation, InputType, Field, Ctx, UseMiddleware, Query, ObjectType } from "type-graphql";
/*Entities*/
...
export class Cards_categoryResolver { 
    ...
    async updateCategory(
        ...
        const result = await getConnection()
            ...
            .where("cd_id = :cd_id and cd_userid = :cd_userid", {cd_id, cd_userid: req.session.userId})
            .execute();
    ...
    async isPackNameExisting(
        ...
        const response = await Cards_category.findOne({where: {cd_userid:req.session.userId, cd_name}});
